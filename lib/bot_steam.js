...
client.on("friendMessage", function(steamID, message) {
		...
		var messageSteamCharacter = message;
		console.log(messageSteamCharacter.slice(0, 8));
		...
		if(messageSteamCharacter.length  >= 3)
		{
			...
			if (messageSteamCharacter.slice(0, 5) == "!dota") {
				//string 1 msg tem q ser Counters
				if (messageSteamCharacter.slice(6, 14) == "counters") {
					
					var heroname = messageSteamCharacter.slice(15, messageSteamCharacter.length);
					//heroname.split(' ','');

					//string 2 msg com o nome do hero
					if (heroname.length >= 0) {

						client.chatMessage(steamID, hero_dados.getDataHero(heroname));
			...
			else if (messageSteamCharacter.slice(0, 8) == "!contato")
			{
				client.chatMessage(steamID, "Contato:\ngabrielcarlos91@hotmail.com");
			}
			else if (messageSteamCharacter.slice(0, 6) == "!sobre")
			{
